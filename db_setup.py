...
def find_trains(station_start, station_end, passengers, day):
    ...
    if weekdays < 5:
        ...
        weekdays = 0
    for instance in session.query(Trains).filter(Trains.train_direction == trip_direction).filter(
                    Trains.train_days == weekdays):
        trains_free[instance.train_id] = 500
        for seg in segments:
            seats = session.query(Seats_Free).filter(Seats_Free.train_id == instance.train_id).filter(
                Seats_Free.seat_free_date == day).filter(Seats_Free.segment_id == seg).first().freeseat
...
def create_reservation_and_trips(train_id, departure_station, departure_time, arrival_station, passengers, booker, day):
    ...
    reservation = session.query(Reservations).filter(Reservations.passenger_id == passenger_id).filter(
        Reservations.date == date).first()
    ...
    for idx, passenger_type in enumerate(passengers):
        ...
        for i in range(passenger_type):
            trip = Trips(trip_time_start=day, trip_station_start=station_start_id, trip_station_end=station_end_id,
                         fare_type=rate_tuple[0], fare=base_fare * rate_tuple[1], train_id=train_id,
...
def get_station_train_departure(train_id, station_id):
    return session.query(Stops_At).filter(Stops_At.train_id == train_id,
                                          Stops_At.station_id == station_id).first().time_out


def get_station_train_arrival(train_id, station_id):
    return session.query(Stops_At).filter(Stops_At.train_id == train_id,
                                          Stops_At.station_id == station_id).first().time_in
...
def increase_seat_free(train_id, start_seg_id, end_seg_id, seat_free_date):
    ...
    for seg_id in range(start_seg_id, end_seg_id + 1):
        # get segment
        seg = session.query(Seats_Free).filter(Seats_Free.train_id == train_id, Seats_Free.segment_id == seg_id,
                                               Seats_Free.seat_free_date == seat_free_date).first()
...
def get_my_trips(email):
    ...
    for res in reservations:
        ...
        station_start_name = session.query(Stations).filter(
            Stations.station_id == station_start_id).first().station_name
...
print(get_my_trips('test@gmail.com'))
